...
public extension Reactive where Base: UIViewController {
    ...
    var isDismissing: ControlEvent<Bool> {
        ...
        /// Rx observable, triggered when the ViewController is being popped from
        /// the NavigationControllerStack
    var didPop: Observable<Void> {
        let viewDidDisappearObservable = self.base.rx.viewDidDisappear.map { [weak base] _ in base }
        
        return viewDidDisappearObservable
            .map { $0?.isMovingFromParent }
            .filterNil()
            .filter { $0 }
            .map { _ in Void() }
    }
